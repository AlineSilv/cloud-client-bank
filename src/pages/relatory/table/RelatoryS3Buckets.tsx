...
const RelatoryS3Buckets: React.FC<RelatoryS3BucketsProps> = ({ data }) => {
  ...
  const handleColumnToggle = (column: keyof S3) => {
    ...
    const exportToJSON = () => {
      ...
      saveAs(blob, "relatorio_s3.json");
    };
  
    // Função para exportar os dados para PDF
    const exportToPDF = () => {
      const doc = new jsPDF();
      doc.text("Relatório de S3 Buckets", 20, 20);
      ...
      autoTable(doc, {
        head: [["Account", "Bucket Name", "Region", "Life Cycle Rules", "Size Bytes", "Storage Class"]],
        body: tableData,
      });
    
      doc.save("relatorio_s3.pdf");
    };
    ...
    const exportToExcel = () => {
      ...
      XLSX.utils.book_append_sheet(wb, ws, "S3 Buckets");
      XLSX.writeFile(wb, "relatorio_s3.xlsx");
    };
      ...
      <Table>
        <Thead>
          <Tr>
            ...
            {selectedColumns.BucketName && <Th>Bucket Name</Th>}
            {selectedColumns.Region && <Th>Region</Th>}
            {selectedColumns.LifecycleRules && <Th>Lifecycle Rules</Th>}
            {selectedColumns.SizeBytes && <Th>Size (Bytes)</Th>}
        ...
        <tbody>
          {currentItems.map((bucket, index) => (
            <Tr key={index}>
              {selectedColumns.Account && <Td>{bucket.Account}</Td>}
              {selectedColumns.BucketName&& <Td>{bucket.BucketName}</Td>}
              {selectedColumns.Region && <Td>{bucket.Region}</Td>}
              {selectedColumns.LifecycleRules && <Td>{bucket.LifecycleRules ? "Configurado" : "Não Configurado"}</Td>}
              {selectedColumns.SizeBytes && <Td>{bucket.SizeBytes}</Td>}
              {selectedColumns.StorageClass && <Td>{bucket.StorageClass}</Td>}
            </Tr>
          ))}
        </tbody>
      </Table>
              ...
              <ModalBody>
                {Object.keys(selectedColumns).map((column) => (
                  <div key={column}>
                    <Checkbox
                      type="checkbox"
                      checked={selectedColumns[column as keyof S3]}
                      onChange={() => handleColumnToggle(column as keyof S3)}
                    />
                    <label>{column}</label>
                  </div>
                ))}
