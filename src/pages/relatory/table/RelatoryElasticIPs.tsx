...
const RelatoryElasticIPs: React.FC<RelatoryElasticIPsProps> = ({ data }) => {
  ...
  const handleColumnToggle = (column: keyof ElasticIP) => {
      ...
      const exportToJSON = () => {
        ...
        saveAs(blob, "relatorio_elasticip.json");
      };
    
      // Função para exportar os dados para PDF
      const exportToPDF = () => {
        const doc = new jsPDF();
        doc.text("Relatório de ElasticIP", 20, 20);
        ...
        autoTable(doc, {
          head: [["Account", "PublicIP", "Region","InstanceID"]],
          body: tableData,
        });
      
        doc.save("relatorio_elasticip.pdf");
      };
      ...
      const exportToExcel = () => {
        ...
        XLSX.utils.book_append_sheet(wb, ws, "ElasticIP");
        XLSX.writeFile(wb, "relatorio_elasticip.xlsx");
      };
  ...
  return (
    <Container>
      <ContentTable>
        <DescriptionBox>
                  <LabelBox>
                  <h3>Relatório de Elastic IPs</h3>
      ...
      <Table>
        <Thead>
          ...
          {selectedColumns.PublicIP && <Th>Elastic IP</Th>}
        ...
        <tbody>
          {currentItems.map((ip, index) => (
            <Tr key={index}>
              {selectedColumns.Account && <Td>{ip.Account}</Td>}
              {selectedColumns.PublicIP && <Td>{ip.PublicIP}</Td>}
              {selectedColumns.Region && <Td>{ip.Region}</Td>}
              {selectedColumns.InstanceID && <Td>{ip.InstanceID}</Td>}
            </Tr>
          ))}
        </tbody>
      </Table>
                  ...
                  <ModalBody>
                    {Object.keys(selectedColumns).map((column) => (
                      <div key={column}>
                        <Checkbox
                          type="checkbox"
                          checked={selectedColumns[column as keyof ElasticIP]}
                          onChange={() => handleColumnToggle(column as keyof ElasticIP)}
                        />
                        <label>{column}</label>
                      </div>
                    ))}
